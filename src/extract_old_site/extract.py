...
def run_extraction(config, version="dig"):
    # Set up variables from config
    dig_parent_dir = config['digParentDirPath']
    dig_dir = None
    # Assumes both dig and digpro are stored in the same parent directory
    if version == "dig":
        dig_dir = pathlib.Path(dig_parent_dir) / "dig"
    elif version == "digpro":
        dig_dir = pathlib.Path(dig_parent_dir) / "digpro"
    ...
    if output_dir_path == "Default":
        output_dir_path_obj = pathlib.Path(__file__).parent.parent.parent / "jsons" / version
    else:
    ...
    for partname in text_partnames:
        ...
        data = standard_text_chapter.extract_standard_part(partname, dig_dir, file_ops.readfile)
        if partname == 'part0':
            data = standard_text_chapter.reextract_title_page(data, dig_dir, file_ops.readfile)
        write_file(data, output_filename)

    # Run getting started extraction
    print("Extracting Getting Started... ...")
    getting_started_data = getting_started.extract_getting_started(dig_dir, file_ops.readfile)
    ...
    primer_data = archaeology_primer.extract_entire_primer(dig_dir, file_ops.readfile)
    ...
    excavations_pages = excavation_details_page.extract_all_exc_pages(dig_dir, file_ops.readfile)
    ...
    images = image_page.extract_all_images(dig_dir, file_ops.readfile)
    ...
    descriptions = feature_descriptions.extract_descriptions(dig_dir, file_ops.readfile)
    ...
    refs = references.extract_all_references(dig_dir, file_ops.readfile)
    ...
    data_downloads_data = data_downloads.extract_data_downloads(dig_dir, file_ops.readfile)
    write_file(data_downloads_data, output_dir_path_obj / "dataChapter.json")

    # Run tables extraction
    print("Extracting tables... ...")
    table_info = tables.extract_all_tables(dig_dir, file_ops.readfile)
    table_strings = table_info['tables']
    table_html_paths_to_nums = table_info['htmlPathsToTableFileNums']
    table_image_paths_to_figure_nums = tables.extract_all_table_image_htmls(dig_dir, file_ops.readfile)
    ...
    print("Extracting appendix A... ...")
    artifacts_summary = artifacts.extract_all_of_artifacts_dir(dig_dir, file_ops.readfile)
    print("Extracting appendix B... ...")
    artifacts_details = artifacts.extract_appendix_b(dig_dir, file_ops.readfile)
    art_images = artifacts.extract_all_artifacts_images(dig_dir, file_ops.readfile)
