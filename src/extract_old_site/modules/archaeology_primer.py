...
def extract_primer_page(html_string, dig_parent_dir, current_page_name, readfile):
    ...
    if soup.map:
        ...
        del form.input['size']
        form_str = str(form)
        map_img = Path(os.path.normpath(Path('/dig/html/primer') / soup.img['src'])).as_posix()
        ...
        for filename in files:
            file_html = readfile(filename, Path(dig_parent_dir) / "dig/html/primer")
    ...
    for a in a_tags:
        if 'javascript:opennewwindow' in a['href']:
            a['href'] = '/dig/html/primer/' + a['href'].split('(')[-1].replace("'", "").split(')')[0]
        else:
            a['href'] = Path(os.path.normpath(Path('/dig/html/primer') / a['href'])).as_posix()
...
def extract_entire_primer(dig_parent_dir, readfile):
    primer_dir = Path(dig_parent_dir) / 'dig/html/primer'

    primer = {
        "path": '/dig/html/primer',
        "modules": None,
        "pages": {}
    }
    videos = {}
    ...
    for filepath in primer_dir.iterdir():
        if filepath.name == 'contents.html':
            ...
            videos['/dig/html/primer/' + filepath.name] = video_info
        else:
            raise Exception("unknown file " + filepath + " found.")

    for primer_html_name in primer_htmls_by_page_num.values():
        page_stuff = extract_primer_page(readfile(primer_html_name, Path(dig_parent_dir) / "dig/html/primer"), dig_parent_dir, primer_html_name, readfile)
...
def extract_video_page(html_string):
    ...
    return {
        "path": Path(os.path.normpath(Path('/dig/html/primer') / link)).as_posix(),
        "caption": caption
    }
            ...
            if str(content).strip() != '':
                if content.name == 'font' and content.text:
                    ...
                    if modules[-1]['path'] == None:
                        modules[-1]['path'] = "/dig/html/primer/" + content['href']
                    modules[-1]['sections'].append({
                        "name": content.text,
                        "path": "/dig/html/primer/" + content['href'],
                        "subsections": [],
