...
def extract_top_level_table_html(html_string, dig_dir_str, readfile):
    """Extract all info from a table*.html page."""
    soup = BeautifulSoup(html_string, 'html5lib')
    frames = soup.find_all('frame')
    header_html = readfile(frames[0]['src'], Path(dig_dir_str) / "html/tables")
    table_body_html = readfile(frames[1]['src'], Path(dig_dir_str) / "html/tables")
...
def extract_all_tables(dig_dir_str, readfile):
    """Extract all tables from /html/tables as strings."""
    table_dir = Path(dig_dir_str) / "html/tables"
    table_pages_by_num = {}
    ...
    for page_num, table_pages in table_pages_by_num.items():
        ...
        for table_page in table_pages:
            if page_num not in tables:
                ...
                htmlPathsToTableFileNums["/html/tables/" + table_page] = page_num
            else:
                table_info = extract_top_level_table_html(
...
def extract_table_image(html_string):
    ...
    soup = BeautifulSoup(html_string, 'html5lib')
    path = Path("/html/tables") / soup.body.img['src']
...
def extract_all_table_image_htmls(dig_dir_str, readfile):
